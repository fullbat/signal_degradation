...
cdef extern from "signal_degradation_c.cpp":
    int trk2dictionary(
...
cpdef linear_scaling( int array_size, int start_value, int end_value ):
    cdef np.float32_t [::1] array = np.zeros( array_size, dtype=np.float32 )
    cdef int i

    for i in range(array_size + 1):
        array[i] = 1.0 - i / array_size

    return array

cpdef exponential_scaling( int array_size, int start_value, int end_value ):
    cdef np.float32_t [::1] array = np.zeros( array_size, dtype=np.float32 )
    cdef int i

    for i in range(array_size + 1):
        array[i] = 1.0 - np.exp( -i / array_size )

    return array

cpdef compute_tdi_scaled( np.uint32_t[::1] v, np.float32_t[::1] l, np.uint32_t[::1] p, np.float32_t [::1] scaling, int nx, int ny, int nz, int verbose):
    cdef np.float32_t [::1] tdi = np.zeros( nx*ny*nz, dtype=np.float32 )
    cdef unsigned long long i=0
    print(f'v.size = {v.size}, l.size = {l.size}, p.size = {p.size}, scaling.size = {scaling.size}')
    with ProgressBar(total=v.size, disable=(verbose in [0, 1, 3]), hide_on_exit=True) as pbar:
        for i in range(v.size):
            tdi[ v[i] ] += l[i] * scaling[ p[i] ]
            pbar.update()
...
cpdef run( filename_tractogram=None, path_out=None, filename_peaks=None, filename_mask=None, filename_ISO=None,
            ...
            keep_temp=False, verbose=3, scaling_array=None
            ):
    ...
    p = np.fromfile( join(path_out, 'dictionary_IC_pos.dict'), dtype=np.uint32 )


    cdef np.float32_t [::1] niiTDI_mem = np.zeros( Nx*Ny*Nz, dtype=np.float32 )
    # niiTDI_mem = compute_tdi_scaled( v, l, p, Nx, Ny, Nz, verbose )
    ...
    cdef np.float32_t [::1] scaling_arr
    if scaling_array is not None:
        scaling_arr = scaling_array
    else:
        scaling_arr = linear_scaling( 256, 0, 1)

    cdef np.float32_t [::1] niiTDI_mem_scaled = np.zeros( Nx*Ny*Nz, dtype=np.float32 )
    niiTDI_mem_scaled = compute_tdi_scaled( v, l, p, scaling_arr, Nx, Ny, Nz, verbose )
    niiTDI_img_scaled_save = np.reshape( niiTDI_mem_scaled, (Nx,Ny,Nz), order='F' )

    niiTDI_scaled = nibabel.Nifti1Image( niiTDI_img_scaled_save, TDI_affine )
    niiTDI_scaled_hdr = _get_header( niiTDI_scaled )
    nibabel.save( niiTDI_scaled, join(path_out,'dictionary_tdi_scaled.nii.gz') )
